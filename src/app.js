    ...
    for (let y = circle.y; y < circle.bottom; y += 2) {
        for (let x = circle.x; x < circle.w; x += 2) {
            ...
            if (grid[gridY] && grid[gridY][gridX]) {
                ...
                if (
                    modeSelector.mode === "colour" ||
                    modeSelector.mode === "nuts"
                ) {
...
function lockPieceIn() {
    if (piece.type === "seed") {
        ...
        getPieceGridCoords(piece, grid).forEach(([x, y]) => {
            if (grid[y])
                grid[y][x] = { colour: getColour(piece, { dim: true }) };
        });
    }
    piece = null;
}
    ...
    if (event === "go nuts") {
        ...
        if (piece && !mainMenu) {
            switch (event[1]) {
                case "left":
                    if (
                        doesCollide({ ...piece, x: wrapX(piece.x + 1) }, grid)
                    ) {
                        // clunk
                    } else {
...
function showMainMenu(message, etc) {
    ...
    */
    ...
    a.innerHTML = `<div class="main-menu">
